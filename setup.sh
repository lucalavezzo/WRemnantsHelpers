...
# Agent bootstrap: ensure the workspace-root CLAUDE.md points at the hub (this repo's AGENTS.md).
_ROOT_CLAUDE="$(dirname "${MY_WORK_DIR}")/CLAUDE.md"
_WANT="@WRemnantsHelpers/AGENTS.md"
if [[ ! -f "${_ROOT_CLAUDE}" ]] || ! grep -qF "${_WANT}" "${_ROOT_CLAUDE}" 2>/dev/null; then
    {
    ...
    echo "Wrote agent bootstrap ${_ROOT_CLAUDE} -> ${_WANT}"
fi
unset _ROOT_CLAUDE _WANT
echo
echo "--- PySR/Julia"
