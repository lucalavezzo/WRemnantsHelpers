...
unfolding_command="combinetf2_fit.py ${unfolding_combine_file} -o ${output} --binByBinStatType normal -t -1 --doImpacts --globalImpacts --saveHists --computeHistErrors --computeHistImpacts --computeHistCov -m Select ch0_masked 'helicitySig:slice(0,1)' --postfix asimov ${extra_fit}"
...
setup_command="python $WREM_BASE/scripts/combine/setupCombine.py -i $input_file -o ${output} --fitresult ${unfolding_fitresult} 'Select helicitySig:slice(0,1)' ch0_masked --fitvar ptVGen-absYVGen-helicitySig --axlim -1000 1000 -1000 1000 -1 0 --fitAlphaS --postfix theoryfit --baseName prefsr"
...
combine_command="combinetf2_fit.py ${combine_file} -o ${output} --doImpacts --globalImpacts --saveHists --computeHistErrors --computeVariations --chisqFit --externalCovariance -t -1 -m Project ch0 ptVGen -m Project ch0 absYVGen ${extra_fit}"
