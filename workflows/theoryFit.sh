...
theory_setup_command="python ${WREM_BASE}/scripts/rabbit/feedRabbitTheory.py ${unfolding_fitresult} --predGenerator 'scetlib_dyturbo' -o ${output_dir} --systematicType log_normal ${extra_setup}"
...
output=$(dirname "$theory_file")
echo "Output: $output"
echo

fit_command="rabbit_fit.py ${theory_file} -o ${output_dir} --externalCovariance --doImpacts --chisqFit --globalImpacts --saveHists --computeVariations --computeHistErrors"
