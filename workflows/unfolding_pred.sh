...
unfolding_command="rabbit_fit.py ${unfolding_combine_file} -o ${output} --binByBinStatType 'normal-multiplicative' -t -1 --doImpacts --globalImpacts --saveHists --computeHistErrors --computeHistImpacts --computeHistCov -m Select 'ch0_masked' 'helicitySig:slice(0,1)' --postfix asimov_unfolded ${extra_fit}"
