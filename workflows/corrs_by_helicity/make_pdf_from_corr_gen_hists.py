...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        "--outdir",
        type=str,
        default=f"{os.environ['MY_OUT_DIR']}/{datetime.now().strftime('%y%m%d')}_pdfsFromCorrByHelicity/",
    )
    parser.add_argument(
        "--skim",
        action="store_true",
...
def main():
    ...
    for pred in args.preds:
        ...
        if args.skim:
            skim_command = f"python {os.environ['MY_WORK_DIR']}/scripts/open_narf_h5py.py {args.outdir}/w_z_gen_dists_{pred + "Corr"}_maxFiles_m1.hdf5 --filterHistsRegex '^(.*VarsCorr.*|nominal_gen_pdf_uncorr)$' --outfile {args.outdir}/w_z_gen_dists_{pred + "Corr"}_maxFiles_m1_skimmed.hdf5"
