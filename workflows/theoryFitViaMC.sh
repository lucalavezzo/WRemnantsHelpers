...
if [ -z "$1" ]; then
    echo "Usage: unfolding_2D.sh <input_file> <unfolding_fitresult> -o <output_dir> -e <extra setupRabbit arguments> -f <extra rabbit_fit arguments> -p <postfix>"
    exit 1
fi
...
while getopts "u:o:e:f:p:" opt; do
    case $opt in
        o)
            output_dir=$OPTARG
            ;;
...
setup_command="python $WREM_BASE/scripts/rabbit/setupRabbit.py -i $input_file -o ${output_dir} --fitresult ${unfolding_fitresult} 'Select helicitySig:0' 'ch0_masked' --fitvar ptVGen-absYVGen --noi alphaS --postfix ${postfix} --presel helicitySig -1.0j 0.0j --baseName prefsr --npUnc LatticeEigvars --pdfUncFromCorr ${extra_setup}"
