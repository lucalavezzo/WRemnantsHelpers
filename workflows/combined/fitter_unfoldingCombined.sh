...
setup_command="python ${WREM_BASE}/scripts/rabbit/setupRabbit.py -i ${input_file_Z} ${input_file_W} -o $(dirname ${unfolding_fitresult}) --fitresult ${unfolding_fitresult} CompositeModel --fitvar ptVGen-absYVGen-helicitySig absEtaGen-ptGen-qGen --noi alphaS wmass --postfix theoryfitViaMC --baseName prefsr --select 'helicitySig -1j 0j'"
