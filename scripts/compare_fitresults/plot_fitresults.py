...
def get_default_colors():
    ...
    return ['blue', 'red', 'green', 'orange', 'purple', 'brown', 'pink', 'gray', 'olive', 'cyan']
...
def add_atlas_results(ax, show_atlas):
    ...
    for result_name, data in atlas_results.items():
        ...
        ax.text(0.02, data['value']*1.001, data['label'], 
               fontsize=14, color='black', ha='left', va='bottom', rotation=0)
...
def main(args):
    ...
    for i, name in enumerate(uncertainties.keys()):
        ...
        ax.scatter(i, uncertainty, 
                  marker=result['marker'], 
                  color=result['color'], 
                  s=100,
                  label=result['label'] if 'show_in_legend' not in result or result['show_in_legend'] else None)
    ...
    fig.savefig(os.path.join(output_dir, f"fitresults{postfix}.png"), dpi=300, bbox_inches="tight")
    fig.savefig(os.path.join(output_dir, f"fitresults{postfix}.pdf"), bbox_inches="tight")
...
def parse_args():
    parser = argparse.ArgumentParser(description="Plot alpha_S uncertainties from fit results specified in YAML config")
    parser.add_argument("config", help="Path to YAML configuration file")
    parser.add_argument("--showATLAS", action="store_true", help="Show ATLAS results on the plot")
    parser.add_argument("-o", "--output-dir", default=f"{datetime.date.today().strftime("%y%m%d")}_fitresults", help="Output directory for plots (overrides config)")
