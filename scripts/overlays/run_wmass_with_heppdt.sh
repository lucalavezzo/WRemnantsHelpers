...
OVERLAY="${WREM_HEPPDT_OVERLAY:-$HOME/.apptainer/overlays/wmassdevrolling_heppdt.img}"
BIND_PATHS="${WMASS_BIND_PATHS:-/scratch/,/work/,/home/,/ceph/}"

if [[ ! -f "$OVERLAY" ]]; then
    echo "HepPDT overlay not found: $OVERLAY"
    echo "Create it first with: scripts/overlays/install_heppdt_overlay.sh"
    exit 1
fi
