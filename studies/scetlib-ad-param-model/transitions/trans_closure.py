...
def main():
    ...
    ap.add_argument("--abl", type=int, default=0,
                    help="ad_muf_abl mask for the SECOND arm (0 = none; 8 adds "
                         "the c_i1 evolution). Needs a build that has it.")
    ...
    def set_arm(used, abl=0):
        ...
        if hasattr(scetlib_qT.DrellYan, "set_muf_ablate"):
    ...
    ARMS = ((0, 0, "anl0", "shipped (member interpolation only)"),
            (args.mode, args.abl, "anl1",
             f"analytic muF evolution, mode {args.mode}, abl {args.abl}"))
    cen = {}
    for used, abl, tag, _ in ARMS:
        set_arm(used, abl)
        cen[tag] = model({})
