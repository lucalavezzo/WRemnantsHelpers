#!/bin/bash
# Container entry for 260910-corr-anchor -- a VERBATIM copy of
# ../260908-fit-770/incontainer.sh, so the Asimov arm here is directly
# comparable to AS770. D deliberately still points at 260908-fit-770: its
# lib/ holds the fixed scetlib_tf.py (hvp zero-seed skip) that arm ran with,
# and without it the postfit Hessian pass costs ~77x more. Read-only use of
# another task's directory; nothing here writes to it.
#
# NB the anchor-check matrix (run_anchor_checks.sh) does NOT go through this
# file: it only constructs the model, never runs a Hessian pass, so the
# zero-seed skip is irrelevant there and the snapshot's own scetlib_tf.py is
# what it used.
#
