...
- **2026-09-10** alphaS is now blinded ADDITIVELY, restoring what the template treatment gave us: [260910-blinding](260910-blinding/LOGBOOK.md). rabbit blinds POIs multiplicatively (right for `Mu`, a yield scale factor centred at 1), which divided the reported **sigma** by a random factor -- only the relative uncertainty survived. `pdfAlphaS` was a NOI and got additive blinding; a model parameter cannot be a NOI, so alphaS inherited the wrong form. Fixed opt-in (`blind_additive`, default False) in rabbit `0f64bbb` on `blinding-additive` plus `589f790b` here. The 2026-09-09 catastrophe was separately a START-POINT bug -- rabbit initialises `x` to `xparamdefault` in the BLINDED frame -- already fixed by `8f6af64f` and now made structural. **Validated on a real blinded data fit**: EDM 1.386e-03 (against 0.608 for the previous free-NP attempt), sigma(alpha_s) = 0.00132 readable with the central value never looked at, 3673 card nuisances impeccable (zero beyond 2 sigma). Two findings for the NP sector: `lambda2_nu` and `lambda4` go NEGATIVE in physical units, and the 47-parameter block has condition number 4.2e4 with **degeneracy 1.08e5** -- larger than the condition number, so the ill-conditioning is genuine near-degeneracy, NOT bad scaling, and preconditioning buys only 2.4x.
...
- **2026-09-10** David's review of rabbit [PR #156](https://github.com/WMass/rabbit/pull/156) (the preconditioner work out of `260909-data-fit`) answered: [260910-pr156-review](260910-pr156-review/LOGBOOK.md). All eight comments verified independently and all eight answered. One filed as non-blocking was a bug on the **default** ridge path, not a doc matter. Per Luca, `--stallRelTol` was split out to [#157](https://github.com/WMass/rabbit/pull/157) (`b49c65c`, 89 tests) and #156 force-pushed as preconditioner-only (`7ae95f9`, 90 tests); old head `9315f24` is recoverable. `WRemnants/rabbit` moved to `combined-156-157` (`f843fa1`), a clean merge of both, 98 tests. Merging the two halves also caught a leak in the split (the reference-matrix timing hunk was in both PRs) — see the task logbook. David then re-verified and argued finding 2 into the code: on the spectral path the whitened block is symmetric-ORTHOGONAL, so true κ ≡ 1 by construction and `_cond_corr` was reporting an artefact growing with block size; #156 now reports true κ (`7b57fac`, 95 tests). Two further rounds followed: he re-verified, then found that **"a usable transform beats none" was false** — ridging an all-negative block is 10.7x worse than the skip it replaced (0 of 7094 improved), so #156 now drops any block whose true κ degrades (`851e442`, 97 tests), with a carve-out for already-singular blocks where κ stops being a measurement. #157 had a reachable bug — a loss pinned at `+inf` was a stall on `main` and went undetected (NaN comparisons) — fixed in `f9bf2f3` (95 tests). Also noted but deliberately NOT changed: the "spectral preserves the sign" rationale is vacuous (Sylvester's law of inertia — any congruence preserves signs). The additive-blinding WIP now has its own branch + worktree. Left open in the thread for David: `_cond_corr` is scale-invariant, so the conditioning numbers the PR added to the log cannot see the failure it exists to fix — [this study's own finding](260909-data-fit/LOGBOOK.md), now shown to hold on the PR's motivating block too.
...
# Logbook entry — 2026-08-26: the frozen node set WORKS, and the barrier diagnosis was wrong

Staged for interleaving into `studies/scetlib-ad-param-model/LOGBOOK.md`.
Decisions staged separately in `DECISIONS_interleave.md` (D-044 .. D-050).
Scripts committed to `studies/scetlib-ad-param-model/nodeset_split/`.

## The question

Luca's proposal: freeze the node set once, then build each PDF member in its own
process against that shared frozen node set, with bins as work items in one
thread pool. The gate was: can `build_pdf_variations` operate on LOADED rules,
or does it need private in-memory state that `prepare`/`build_bin_rules` left
behind?

## Answer: yes, in two steps, and the reason is better than expected

**Read the source first.** `build_pdf_variations` needs the FULL node pool of
each retained outer point, not just the compacted rule: `b[0] = sum_j W0[j]` and
`var.c_val` run over all `ns` pool sites, and the loaded rule's site record
indexes that pool (`flat[si] = off[fi] + (pass ? n_incl[fi] : 0) + st.node`).
The rules blob stores only the compacted grid and the SELECTED sites, so the pool
is not in it, and the library says so by name when you try:
`set_pdf_keep_nodes: neither cache is populated, so there is no geometry to keep.`

**But the outer node set is already in the cache.** In the paired configuration
every production build uses (the "Sharing one outer node set" line),
`_ad_bin_grid` never adapts: it copies `_matched_partner->shared_outer_grid()`,
which is `_fo_bin_grid()`, which is `_fo_cache` — and `save_fo_cache` writes
every node's `(Q, Y, qT, weight)` as raw doubles. So the expensive, discrete,
irreproducible part of the node set is a FILE today and nobody noticed.

What remains to regenerate is only the per-point bT node set, and that is
deterministic: the bT integrator's one mutable member is a double-exponential
abscissa table derived from `_precision` alone, not accumulated history. So:

```
nons.load_fo_cache_bytes(fo)     # FIRST -- the outer grid must be the file's
sigma.prepare(bins, p0)          # the "rewarm": repopulates the bT nodes
sing.load_bin_rules_bytes(rules) # LAST  -- the frozen site selection is final
build_pdf_variations / build_fo_pdf_variations   # this member slice
```

## Demonstration: byte-identical, three ways

4 bins (qT [33,44] and [44,100] x |Y| [0,0.15] and [0.15,0.3]) of the 260820 Z
card, rel 1e-3, `--pdf-eig 2` (8 members, 4 atomic pairs), P = 26.
`compare_caches.py --bytes` on every field of both blobs:

* separate process, loaded node set, all 8 members **==** the originating
  process's own serial member loop — **IDENTICAL** (this is the closure: the
  save/load round trip plus the rewarm reproduces the in-memory geometry)
* merge(members 0:4 ⊕ members 4:8, two processes) **==** one process on the same
  frozen node set — **IDENTICAL** (this is the split)
* the same member range built at `--threads 8` and at `--threads 16` —
  **IDENTICAL** (determinism does not depend on the pool size)

Control, and the reason this is a result rather than a coincidence: three
INDEPENDENT full builds of the identical configuration at the identical
`--threads 8` gave retained sites/bin `[363,368,400,399]`, `[363,365,400,400]`,
`[363,365,400,400]`. Two agreed and one did not — so the D-013 hazard is real and
INTERMITTENT, which is exactly the case that would sometimes coincide and be
silently accepted. Freezing removes the coin flip structurally.

## The number that changes the architecture

A member shard's prologue collapses from 194 s to 1.3-1.9 s at 4 bins
(**150x**), and from 123 s to 0.6 s at qT [1,2] (**205x**). The ratio improves
where the node set is expensive, because the fixed-order ladder is the expensive
part and it is the part that comes from the file. `build_bin_rules` is skipped
entirely. Matched sums reproduce exactly across the round trip
(14.92940684 pb, 2.411335343 pb).

## And the correction: the barrier is real, but it is worth ~7 %, not ~28 %

Instrumented rather than inferred — 1 Hz sampling of running threads and CPU rate
on two 50-bin builds at `--threads 48` (bins:threads = 1.04, the production
ratio), 20 members each. Trace plot:
`~/public_html/alphaS/260826_nodeset_split_barrier/build_utilisation.png`.

| build A: qT 20-100, uniform cost | wall | busy of 48 | core-s lost | drains below 90 % |
|---|---|---|---|---|
| outer node set | 1.18 min | 47.4 (99 %) | 1 % | 0 |
| bin rules | 8.54 min | 46.5 (97 %) | 3 % | 1 |
| resummed members | 4.6 min | 32.8 (68 %) | **32 %** | **exactly 20**, 8.1 s each |
| fixed-order members | 14.9 min | 46.0 (96 %) | **5.3 %** | 20-24, 1.9-2.5 s each |

| build B: qT 1-2 / 5-6 / 10-11 / 16-18 / 44-100, 3.3x cost spread | wall | busy of 48 | core-s lost |
|---|---|---|---|
| outer node set | 3.52 min | 42.0 (**87 %**) | **12.9 %**, one 23 s tail |
| bin rules | 11.1 min | 47.2 (98 %) | 2.5 % |
| resummed members | 4.8 min | 33.6 (70 %) | 30.7 %, exactly 20 drains, 8.8 s each |
| fixed-order members | 19.3 min | 45.8 (95 %) | 5.8 %, 18 drains, 3.3 s each |

**The barrier is exactly what Luca described — one pool drain per member, 20 of
them for 20 members, in both member stages and both builds.** The prediction was
right; the magnitude was not. The drain is ~8 s of a ~14 s resummed member step
but only 1.9-3.3 s of a ~45-60 s fixed-order step, and the fixed-order half is ~95 % of
the member cost at 210 bins. Weighted: **~7 % of the member work**, 7.8 % of
build A end to end (6533 of 84 144 core-s). Not 27.5 %.

Where the rest went: build B's NODE-SET stage runs at 87 % against A's 99 %,
purely from a 3.3x bin-cost spread, and it loses it as one long tail rather than
as periodic drains. At 210 bins qT [0,1] is >27 min against 0.2-1.0 min for every
other ptV shard — a ~30x spread. **The node-set tail is the missing cores.**

## What it buys — and what it does not

Anchored on the measured production stage costs, the 210-bin member work is
~72 000 core-min; at the 96 % packing just measured, on 768 cores that is
**~1.6 h — precisely what the existing 21-way bin split already delivers.** The
8.7 h monolith was `--threads 200`-limited on a 768-core machine, not
barrier-limited. So on one node, at 210 bins, this change buys **no wall clock.**

It buys four other things: the critical path of a bin-sharded partition (the
qT [0,1] shard stops having to run 62 members serially after its 27 min
prologue); a second orthogonal work axis so tiles fit the cores available;
members as partial results across the whole grid; and a legal multi-machine
fan-out of the member loop at a ~4 min per-worker prologue instead of ~2 h.

Projected for the 781-bin grid: the D-041 bracket of 55-80 h was quoted "at 210
threads"; the same work packed at 96 % on 768 cores is **~7-9 h on one node**,
~3.5-4 h on four. Projection, not measurement — that grid has never been built.

## Next

1. Repeat the byte-identity chain at production scale (210 bins, 62 members,
   P = 53) on `build-nak`, the production library. `build-fix` was used here
   only because it is the tree carrying the CT18ZNNLO beamfunc grids.
2. If it holds, add `--nodeset <cache.npz>` to `prepare_cache_for_card.py` (it
   is ~15 lines: the three calls above, in that order, replacing
   `prepare` + `build_bin_rules`) and a `--freeze-only` mode that writes the
   node-set cache with a sidecar. `build_cache_parallel.py --merge-only` already
   merges the member axis and rebuilds the header from `.shard.json`.
3. Refuse to write a node-set-only cache unless the runcard is PAIRED
   (`fo_node_target_rel > 0`); unpaired, `_ad_bin_grid` adapts its own grid,
   nothing persists it, and none of this works.
4. The 781-bin runcard's one-bin `--subset` timing test (D-041) is still owed and
   is now more informative: it should be run in BOTH modes, so it measures the
   freeze cost and the rewarm cost separately.

**Do not disturb the existing shard-and-merge path.** It is validated and it
stays the production route until item 1 passes.

---

## Addendum: the coordinator's narrowed questions (same session)

**Where the member barrier actually lives — the state, not the memory.** The
member OUTPUTS are written straight through: everything a bin produces inside
`_ad_parallel_run` goes directly into its own `Bin_rule::Var` record (`w`, `nd`,
`c_val`, `c_grad`) or into `_fo_var_d[mi]`, and the per-bin working set (`W0`,
`V0`, `G0`, `A`, `b`) is local to the work item. Nothing accumulates across a
member step. But the member INPUT does: `set_pdf_keep_nodes` calls `set_pdf`
(a whole-object reset), then **rebuilds every node-cache `Entry` as a fresh copy**
and swaps them in — so the convolutions live in ONE shared store holding one PDF
member at a time, transiently doubled at every switch, and convolutions are ~96 %
of that store.

So a single-process queue over (bin, member) items is not a scheduling change,
it is a C++ redesign: K members in flight need K convolution stores and the API
cannot express that. The multi-process route gets them for free. Kernel state is
`thread_local` (`ad_state.cpp`) and `s_ad_mutex` is only an API re-entrancy
guard, as the coordinator said — neither needs changing.

**The memory finding, which is the biggest number in this session.** Measured
import by import in-container: bare python 12 MB, +numpy/h5py 48, +`scetlib_qT`
54, **+tensorflow 858**, after `configure` 930 MB. **TensorFlow is 804 MB of
every build process and the builder never calls it** — it arrives transitively
through `wremnants`, exactly like the ~1530 wasted threads of D-030. The
21-process production split carries ~17 GB of resident TF that nothing uses.
Whole-process peaks for scale: 1.36 GB (4 bins, 8 members), 6.82 GB (50 bins,
20 members). Node cache 13 MB/bin and node cache + nominal rules 25 MB/bin at qT 33-100 (clean
two-point fit, intercept ~1.09 GB), much denser at low qT; member records
~1.14 MB/bin/member. Extrapolated, a 210-bin 62-member monolith is ~21 GB and a
781-bin one ~75 GB — so memory alone forbids "one process per member over all
bins" and forces the 2-D tile.

**Recommendation.** Route 2 is not worth it: the barrier it targets is worth ~7 %
and paying for it means rebuilding the node cache to be per-member. Route 1 is
worth it, but as a **2-D tile** — freeze per bin group, then run (bin group x
member group) tiles, each loading its group's frozen node set (~1 s prologue) so
its footprint stays bounded by its bin count. Every piece of machinery for both
axes already exists. And **before either**, drop the transitive TensorFlow import
from the build path: 804 MB and ~1530 threads per process, no physics risk.
...
## 2026-08-26 — the cache build and the cache format move into SCETlib

Staged for `LOGBOOK.md`. Decisions in `DECISIONS_upstream.md` (`U-001`..`U-009`).
**No physics in this entry** — it is a refactor, and its whole value is in not
breaking anything, so most of what follows is the evidence that nothing moved.

### What moved, and where the seam is

The two modules were 1863 lines of WRemnants that had almost nothing WRemnants
in them. `prepare_cache_for_card.py` called only SCETlib's own pybind API
(`sigma.prepare`, `build_bin_rules`, `build_pdf_variations`,
`build_fo_pdf_variations`, `set_pdf_eig_params`, `sigma_binned_batch`, the
saves). `build_cache_parallel.py` imported SCETlib **zero times** while
depending entirely on its internals — it parses and splices the `SCTRULE` and
`SCETFOG` blobs from raw bytes. And the two depended on each other in **both**
directions: the scheduler ran the builder as a subprocess while the builder
imported the scheduler for its merge.

Three pieces now, and the cycle is gone:

| piece | where | what |
|---|---|---|
| the cache FILE | `scetlib-cms/py/scetlib_cache.py` (new) | TF-free writer, blob readers, `merge_bin_caches`, `merge_shards`, `split_pairs`, `copy_runcard` |
| the BUILDER | `scetlib-cms/examples/matched_ad/prepare_cache.py` | `plan_variations`, `build_prologue`, `build_variations`, `write_cache`, `fork_member_build` — driven by an explicit bin list |
| the WRAPPER | `scripts/rabbit/scetlib_ad/prepare_cache_for_card.py` + `build_cache_parallel.py` | gen axes off the card, the runcard, `--subset`, orchestration, thread budgeting |

WRemnants: **1863 -> 842 lines** (849+1014 -> 523+319), of which ~460 is
argparse help and docstring.
SCETlib gains 913 (new module) + ~280 (the builder functions).

The deciding argument was maintenance, not line count: the blob-format knowledge
— which per-rule fields must match bit for bit, that `Bin_rule_opts` is a raw
POD *including uninitialised padding*, that `_Fo_cache::bins` is an
`unordered_map` whose write order is thread scheduling, that a shard's stored
`n_eig` is per-shard and not per-cache — describes a layout defined in
`src/qT`. Downstream, a layout change reached us as wrong answers. Upstream it
moves in the same commit as the layout.

Upstream **MR !10** (`upstream-cache-build` -> `autodiff-sigmaul`),
https://gitlab.cern.ch/scetlib/contrib/scetlib-cms/-/merge_requests/10. No C++
touched, so `sizeof(ad::GlobalData)` does not move and every cache on disk stays
loadable. !8 and !9 untouched.

### TensorFlow is out of the build path (the highest value-per-risk item)

It arrived through `xsec_backend._import_scetlib()`, which fetched
`sl_config`, `sl_variations` **and** `ScetlibCachedXsecTF` from one `try:`
block — so `configure()`, the first thing a build calls, imported `scetlib_tf`
and with it TensorFlow. The only thing that needed it was the save, and the save
is `np.savez_compressed`. Now `_import_cached_xsec()` is separate and
evaluation-only, and the builder writes through `scetlib_cache.save_cache`.

Real 2-bin build, same runcard, same `--threads`, one change apart:

```
                       before    after
tensorflow imported      yes       no
python modules          5165      848
peak RSS (MB)          1404      681
threads at exit         203        90
```

D-030's `TF_NUM_INTRAOP_THREADS`/`TF_NUM_INTEROP_THREADS` export is therefore no
longer needed for a cache build (knowledge note updated). Still right for a tool
that genuinely imports TF without needing its pools; still must NOT be applied
to rabbit fits.

### Evidence that the production path is unchanged

**The call log, over every route.** `default_path_equivalence.py` extended from
3 configurations to 7 — default, `--no-pdf`, `--no-muf`, `--pdf-eig 2`,
`--members 0:2`, a bin subset, `--fork-members 2`. All **SAME**, against the
installed files. This is the right test because byte-identity of a rebuilt cache
is unobtainable (D-013).

**The writer is byte-identical.** Load a real cache, write it back with both
writers: every array matches except 3 bytes at offsets 1566-1568 of the rules
blob. Those are the last three bytes of the `Bin_rule_opts` POD (`[1513, 1569)`)
— and **two runs of the OLD writer differ in the same three**. All 13,025,133
bytes after the header are identical, both comparisons.

**Evaluation is unchanged.** `backend_check` before and after the `scetlib_tf`
delegation differs only in wall-clock timings: same `sum(sigma) = 11.600383`,
same FD agreement 1.15e-09, same `max|H - H^T|/max|H| = 0.00e+00`.

**The bin merge.** Two independently built shards (2 bins + 1 bin, full member
list each), built by the scheduler and merged by `merge_bin_caches`:

```
merged vs shard A (2 common bins)   0.000e+00  bit-identical  value+jacobian, anchor+displaced
merged vs shard B (1 common bin)    0.000e+00  bit-identical  value+jacobian, anchor+displaced
parts vs whole                      0.000e+00  (16.70535994222326 both sides)
three arms, three sums              16.705 / 11.600 / 5.105  -> not a memoised collision
backend_check on the merged         all checks passed (FD 1.16e-09, H symmetry 0.00e+00,
                                    gen-fold sum rule 0.00e+00)
```

**The member merge.** `split_merge_selftest.py` on a real 4-member cache: split
into `[0,2)` and `[2,4)`, merge back — **all 9 arrays byte-identical**.

**The guard that must not weaken.** New `cross_build_guard.py`. Two independent
builds of the same runcard agree on version, struct sizes, fingerprint,
`Bin_rule_opts` fields and anchor — i.e. on everything the C++ loaders check —
while **2 of 2 nominal rules differ byte for byte**. Same-build halves merge;
cross-build halves are refused. That byte comparison is the only thing between a
cross-build member merge and a silently wrong cache.

### Two traps hit, both worth remembering

**`sys.modules[__name__]` is not available to a module loaded by path.** The
knot-spacing scans (`knot_scan/prepare_cache_knots.py`,
`transition_knots/prepare_cache_5knot.py`) work by rebinding
`mod.plan_variations` / `mod.build_variations` and then calling `mod.main()`. If
`main()` had moved upstream, those rebinds would have been silently ignored —
and the failure mode is a build that completes with the DEFAULT knots while its
log claims the override. So `main()` stays in the wrapper and resolves each step
through `_resolve_steps()`, which prefers this module's globals. The first
attempt used `sys.modules[__name__]` and died with `KeyError`, because
`spec_from_file_location` does not register a module name — which is exactly how
those callers load it.

**The private blob names are part of the API.** `split_merge_selftest.py` has to
WRITE a blob in order to split one, so it uses `m._U64`, `m._FO_MAGIC`,
`m._emit_fo_grid`. Those are re-exported alongside the public ones; two readers
of one format is how a merge starts passing against a bug in itself.

### Found in passing

`scetlib-cms/build/` is stale — no `set_matched_partner` in its `libscet-qT.so`
— and `setup.sh` defaults `SCETLIB_BUILD` to it, so anyone taking the default
gets `'scetlib_qT.DrellYan' object has no attribute 'set_matched_partner'`,
which reads like a source bug. Every validation here used
`SCETLIB_BUILD=.../scetlib-cms/build-fix`. Not fixed: the production build is
pinned elsewhere (D-031 snapshots `build-nak`), so whoever owns that should
decide whether to rebuild `build/` or make `incontainer.sh` name the build.

### State

Installed in the shared tree and validated there. `scripts/rabbit/scetlib_ad/`
and `wremnants/postprocessing/scetlib_ad/` are **untracked** in WRemnants, so
the WRemnants half could not be branched; pristine copies are at
`/tmp/scetlib_ad_orig/` (also what the harness diffs against) and in the job
scratch. **These files want committing.** The SCETlib half is on
`upstream-cache-build` in `/work/submit/lavezzo/alphaS/scetlib-upbuild` and
cherry-picked into the shared checkout as `9d633f2` so both trees carry the same
python (the D-029 lesson: uncommitted python in that tree is one `git checkout`
from vanishing).
...
Aggregated over all four directions, in absolute sigma over the bins the
reference resolves, as a ratio to `anl1` (below 1.00 is better):

```
              x2 = 0.35      x2 = 0.75     x2 = 0.55*      x1,x3
  arm        max   mean     max   mean     max   mean     max     mean
  cubic     2.53   3.14    3.92   2.15    4.50   2.70     778      450
  quartic   2.23   2.73    4.49   2.52    4.39   2.59   38338    22178
  bq(0.3)   1.29   1.10    0.97   0.94    0.97   1.06    3.33     2.60
  bq(1)     1.41   1.41    0.95   1.14    1.94   1.49    2.11     2.19
  bq1a      1.76   1.73    1.81   1.46    2.56   1.76    0.67     1.13
  clip      2.62   3.14    4.49   2.52    4.39   2.59    3.66     2.74
  bc(1)     0.70   1.14    1.69   1.36    2.61   1.83    1.33     1.15
```

**Of the 28 (form, direction) pairs, exactly one has a better mean `|dev|` than
the shipped quadratic: `bq(0.3)` on x2 = 0.75, at 0.94.** The best candidate
overall, `bq(0.3)`, is neutral on the three x2 legs and 2.6x worse on the mean of
the x1,x3 leg. Nothing reached the CorrZ closure gate, so that closure was never
run.

At the NEAR-ANCHOR derivative — the regime a fit uses — the trade is starkest:
the two bins carrying the direction, [44,100] (6.0e-03) and [33,44] (4.0e-03),
go from -1.6% / -1.6% to -9.7% / -9.1% under the quartic, in exchange for qT
[20,24] (-48.9% -> -16.8%) on a bin carrying 22x less response. Every form gains
~30 pp on 4% of the response and loses ~8 pp on 90% of it. And the clipped form
is byte-identical to the quartic there, as it must be (near the anchor every node
is inside its own stencil, so the clip is unclipped) — a second implementation
check that passed.
