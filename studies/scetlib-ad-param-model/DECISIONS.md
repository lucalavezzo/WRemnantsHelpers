...
### D-041 — SUPERSEDED by D-C* : the 781-bin cache is ~1 h, not 55-80 h
**The bracket was wrong for the central case.** The whole cost is gen qT < 2 GeV
(0.0-0.2 min/bin above 3 GeV, 1.3 min at [1,1.5], >25 min for one [0,0.5] bin),
and the member loop -- 68% of the shipped cache's 1052 min -- cannot move the
anchor, so `--no-pdf` is the right build for a central-value question. Measured:
the 748-bin fine cache took **56 min 15 s at 200 threads**, 176 CPU-hours,
175.4 MB. Original entry follows for the audit trail.

### D-041 (as written) — the 781-bin cache costs ~55-80 h
...
### D-049 — TRANSITIONS: CLOSED as a question. Ship MR !9 off by default — SETTLED
"closes qT >= 24, does not close qT < 24." Never ship an r'(0)=0 form at mode 1.
**The previous round's diagnosis was wrong, and nobody had measured the quantity
it rested on.** It assumed the residual satisfies r'(0) = r''(0) = 0. Measured
(`conv_probe` gives the convolution at any muF; delta replicates term-for-term
from `muf_evo_coeffs`): **e1 = r'(0) is 13.8% of the node response at the
muF = 1.40 GeV floor** -- and the floor is exactly where qT 18-24 draws its
transition response.
Each candidate's price is then closed-form in the stencil geometry:
`err = (A1-1) e1 D + (A2-1) e2 D^2/2`, reproducing the measured node error to max
0.018 / rms 0.007 across errors spanning 0.000-0.517. A1 reaches **8.02** on the
x1,x3 leg, and 8.02 x 13.8% x the 6-10x RG cancellation IS the -1326% the prior
round saw.
**And the quadratic is provably unique**: A1 = A2 = 1 is two linear equations with
determinant ab(b-a) != 0, so the quadratic Lagrange pair is the ONLY three-point
form that leaves e1 and e2 alone. Knot exactness is a consequence, not a choice.
Gate: 10 arms x 4 directions, x1,x3 first as instructed. **Of 28 (form,
direction) pairs exactly ONE beats the shipped quadratic on the mean**, and it is
neutral on the three x2 legs and 2.6x worse on x1,x3. Every form gains ~30 pp on
4% of the response and loses ~8 pp on 90% of it. The raw cubic is NOT the safe
intermediate (+180426% at [44,100]); the guards prevent blow-ups and buy nothing.
Invariants held on all nine arms: central bit-identical (0.000e+00),
`sizeof(ad::GlobalData)` unmoved.

### D-050 — The low-qT shortfall is NOT in the interpolation — SETTLED (the finding that outlives)
The quadratic reproduces the convolution at the displaced muF to **<= 1.8% of the
node's own response at every node of every leg**, while the sigma-level residual
at qT [20,24] is -32% to -41%. The quartic is **10x worse at node level and 2x
better in sigma** -- it COMPENSATES, it does not correct. A <= 2% per-node change
moves the [28,33] bin by 74 pp, and the sigma-level deviation is not monotone in
how much non-quadratic form is mixed in. **There is nothing to tune**, and the
next investigation should look outside the interpolation entirely.

### D-051 — The upstream ask, with a number — OPEN (for Josh)
Either **mode 3** (measured to divide e1 by 6.7x, which makes every form safe),
or -- cheaper and better -- **store the grid's own d(conv)/dlnmuF as two extra
conv kinds at cache-build time**, which makes r'(0) = 0 true BY CONSTRUCTION
rather than assumed. `conv_probe` already provides the quantity.

### D-052 — Cross-agent hazard, recorded — SETTLED
Two legs of this round had to be re-run because a parallel agent changed
`_import_scetlib`'s signature mid-queue (the upstream cache-build refactor).
Running several agents against one shared tree costs real re-work; the affected
script now tolerates both signatures. Prefer pinned snapshots for any measurement
campaign that spans hours.
...
## Transitions: analytic DGLAP evolution, end to end (agent, 2026-08-26)

NOTE: this agent numbered its entries D-031..D-044, colliding with existing
D-031..D-044. Renumbered here as **T-031..T-044**; cross-references inside the
block use the original numbers -- read them as T-0xx. Webdir keeps the originals.
...
### T-031 — The prior round's runcard-reference measurement EXISTS, was never
...
### T-032 — `node_cval` and the discarded `rule.c_grad` are BOTH CLOSED as
...
### T-033 — The ~9x RG cancellation is now MEASURED at the sigma level, per bin
###          — SETTLED
...
### T-034 — REFUTED: the low-qT loss is NOT a collapsed-stencil numerical
...
### T-035 — REFUTED: the low-qT loss is NOT the model's frozen NONSINGULAR
...
### T-036 — WHAT THE ROUTE DELIVERS. Mode 1 closes the transitions in
...
### T-037 — THE RUN-TO-RUN SCATTER OF THIS MEASUREMENT, and what it licenses
###          — SETTLED
...
### T-038 — REFUTED: the qT [20,24] shortfall is NOT the integration target
...
### T-039 — The c_i1 evolution term is REQUIRED by the O(alphas^2) muF
...
### T-039b — The same at the NEAR-ANCHOR derivative (x2 = 0.55, what a FIT
...
### T-040 — THE UNIFYING IDENTITY: the analytic+residual construction is BLIND
...
### T-041 — What is LEFT at qT <= 24, bounded from two sides that DISAGREE
...
### T-042 — SHIP mode 1 + the c_i1 term, OFF BY DEFAULT; do NOT build a mode-3
...
### T-043 — THE LOW-qT RESIDUAL IS THE CONSTRUCTION, AND IT IS DEMONSTRABLY
...
### T-044 — REJECTED: clamping the interpolation coordinate to the stencil
...
### T-043b — The same at the NEAR-ANCHOR derivative, and the per-bin winner map
###           — SETTLED
...
### T-043c — CORRECTION to D-043's status: the quartic DIVERGES on the x1,x3
...
### T-043d — The construction map holds on all THREE x2 legs — SETTLED
...
## Transitions: is there a safe interpolant form? (agent, 2026-08-26) -- NO
...
### All four directions, in absolute sigma

max `|dev|` and mean `|dev|` over the bins whose true response exceeds 1e-4 of
sigma, as a ratio to `anl1` (mode 1 + the quadratic, the shipped candidate).
**Below 1.00 is better:**
...
`*` x2 = 0.55 is the NEAR-ANCHOR derivative, what a FIT uses; the other three are
the FINITE variations the templates carry.

**Of the 28 (form, direction) pairs, exactly ONE has a better mean `|dev|` than
the shipped quadratic: `bq(0.3)` on x2 = 0.75, at 0.94.** Four more beat it on
the max, which is a single-bin statistic. Every other entry is a regression.

The best candidate overall is `bq(0.3)`, the conditioning-guarded quartic at the
tightest tolerance: essentially neutral on all three x2 legs and **2.6x worse on
the mean and 3.3x worse on the max of the x1,x3 leg** — the leg the round was
told to gate on. The success criterion (all four directions improving, none
regressing) is met by no form, so **the CorrZ closure was never run: nothing
reached that gate.**

Bin by bin the trade is always the same, and always bad because of where the
response lives. At x2 = 0.55 the two bins that carry the direction are [44,100]
(6.0e-03) and [33,44] (4.0e-03), where `anl1` is -1.6% and -1.6%; the quartic
takes them to -9.7% and -9.1% and buys qT [20,24] (-48.9% -> -16.8%), a bin
carrying 2.7e-04 — **22x less**. Every form gains ~30 pp on 4% of the response
and loses ~8 pp on 90% of it.

**A second implementation check that passed.** At x2 = 0.55 the clipped form is
identical to the quartic in every bin to every digit — required, since near the
anchor every node sits inside its own stencil, so `min((D/m)^2, 1)` is unclipped.
The same way the previous round's coordinate clamp (bit 32) was correctly a no-op
there.
