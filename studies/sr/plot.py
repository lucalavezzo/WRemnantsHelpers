...
parser.add_argument("-n", type=int, default=5, help="Show top -n equations.")
parser.add_argument(
    "--axes",
    nargs="+",
    type=str,
...
for sortby in sortbys:
    ...
    h_pred.view(flow=False)[...] = np.stack([pred, np.zeros_like(pred)], axis=-1)
    h_preds.append(h_pred)
    best_idx = pysr.sr.idx_model_selection(eqns, "best")
    labels.append(
        f"{sortby}={round(eqns[sortby][best_idx],round_digits)} (Best): ${model.latex()}$"
    )
    ...
    for i in sorted_args:
        ...
        h_preds.append(h_pred)
        labels.append(
            f"{sortby}={round(eqns[sortby][i],round_digits)}: ${model.latex(i)}$"
        )
    ...
    h_plot.view(flow=False)[...] = np.stack([vals, np.zeros_like(vals)], axis=-1)
    ...
    plot_tools.save_pdf_and_png(
        args.outdir,
        f"results_{sortby}_{args.postfix}" if args.postfix else f"results_{sortby}",
        fig,
    )
