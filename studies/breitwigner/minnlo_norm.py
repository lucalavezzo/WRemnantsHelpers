...
def main():
    ...
    parser.add_argument(
        "--axes", nargs="+", type=str, default=["pt", "eta"], help="Axes to plot."
    )
    parser.add_argument(
        "--filterProcs",
        nargs="+",
    ...
    with h5py.File(args.infile, "r") as h5file:
        ...
        for proc in procs:
            ...
            corrections = np.array(
                [
                    h[{"massShift": i}].sum() / h_bw[{"breitwignerVar": i}].sum()
            ...
            clean_massShift_values = [
                m.strip("massShiftW").strip("MeV") for m in massShift_values
            ]
            ...
            ax.plot(
                clean_massShift_values,
                xsec_per_massShiftBW,
                marker="o",
                label="breitwignerVar",
            )
            ax.set_xlabel("massShift")
            ax.set_ylabel("Integrated cross section")
            ax.legend()
            plt.setp(ax.get_xticklabels(), rotation=90)
            plot_tools.save_pdf_and_png(args.outdir, "massShift_xsec", fig)
